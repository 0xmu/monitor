...
export async function getPositionIDsToLiquidate(productId, price, isLong) {
	...
	let varText;
	if (isLong) {
		varText = `isLong: true, liquidationPrice_gt: ${Math.round(price*1.01)}`;
	} else {
		varText = `isLong: false, liquidationPrice_lt: ${Math.round(price*0.99)}`;
	}
...
export async function getPositions(_sort) {
	let filter;
	if (_sort == 'amount') {
		filter = 'orderBy: amount, orderDirection: desc, first: 200';
	} else {
